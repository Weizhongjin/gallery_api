...
def editor_token(db):
    uid = uuid.uuid4().hex[:6]
    user = User(
        email=f"editor-{uid}@example.com",
        password_hash=hash_password("pw"),
        name="Editor",
...
def test_admin_cannot_downgrade_self_from_admin(client, db):
    admin = User(
        email=f"admin-safe-{uuid.uuid4().hex[:6]}@example.com",
        password_hash=hash_password("pw"),
        name="Admin Safe",
        role=UserRole.admin,
    )
    ...
    assert response.json()["detail"] == "不能移除当前登录管理员的管理员角色"


def test_last_admin_cannot_be_deactivated(client, db):
    admin = User(
        email=f"admin-last-{uuid.uuid4().hex[:6]}@example.com",
        password_hash=hash_password("pw"),
        name="Admin Last",
        role=UserRole.admin,
    )
